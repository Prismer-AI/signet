...
- **signet-cli**: `delegate create --max-calls N` / `--spend-limit <amount>,<currency>` sugars; all sign paths build gates from the local audit log and revocation file (single-host honesty; read failures surface as warnings, never silently block).

### Changed

- **signet-core**: Revocation — issuer-signed `RevocationRecord`s over both delegation tokens and authorization decisions (v0.11 S3). Two-valued status (`Revoked`/`Unknown`): absence of a record is never evidence of validity; a provable `Valid` awaits signed complete lists (Phase 2/3, additive). Only the artifact's issuer can revoke (delegator/authority key, checkable offline against embedded pubkeys); malformed or forged records are hard errors; records for unknown artifacts are ignored. `sign_revocation`, `verify_revocation_record`, `check_revocation`, plus `fs_ops` (`~/.signet/revocations.jsonl`, atomic temp+rename appends, per-line malformed reporting). New error variant `DelegationRevoked`.
- **signet-cli**: `signet revoke --chain <file> [--token-id]` (leaf by default) / `--decision <file>` — resolves the issuer key from the keystore by pubkey and refuses non-issuers; `signet revocations check [--prune --yes]` validates every line and prunes malformed ones only after explicit confirmation. `delegate sign` and `sign --chain/--decision` run a fail-closed gate against local records. `delegate verify-auth --revocations <file> [--require-revocation-known]` prints the explicit status line: `revoked` exits nonzero, `unknown` exits zero unless the strict flag is set. Evidence bundles now include `revocations.jsonl` when present and `--restore` verifies each bundled record is well-formed, so off-host auditors distinguish `revoked` from `unknown`.
- **signet-python**: `DelegationRevokedError` mapping.

### Changed

- **signet-core**: `AuthorizationDecision` — the authority-signed grant object closing "agent signed ≠ agent authorized" (v0.11 S2). Independent `DecisionType` (wire-compatible with `RuleAction`), polymorphic `DecisionBasis` (policy implemented; approval/delegation/risk/external schema-ready), `CanonicalIntent`/`intent_hash` binding (transport lowercased; sign-time correlation fields excluded so two-step pre-authorization works), `credential_ref` claim slot (never resolved in 0.11). New APIs: `authorize`, `verify_decision`, `verify_decision_trusted`, `verify_decision_for_action`, `sign_with_decision` (two-step, v1 or v4 via optional chain), `sign_with_policy_authority` (one-step; `receipt.policy` omitted — the decision's Policy basis subsumes the attestation). Sign flows accept only allow decisions; replay onto a different action is rejected by intent binding.
- **signet-cli**: `signet authorize` (authority-side pre-authorization with `--policy/--ttl/--max-calls/--credential-ref`), `sign --decision/--chain/--authority-key/--authority-principal`, `verify --authority <name-or-pubkey>` (strict: receipt must carry a decision signed by that key).
- **signet-core**: `verify()`/`verify_any` are decision-aware — the authority signature, intent binding, subject corroboration, and earliest-wins expiry (receipt `exp` vs decision `expires_at`, error names the source) are checked whenever a decision is embedded. `verify_any`'s v4 branch now uses full `verify()` semantics instead of signature-only.
- **signet-wasm / @signet-auth/core**: `wasm_intent_hash`, `wasm_authorize_with_policy`, `wasm_verify_decision(_for_action)`, `wasm_sign_with_decision`; TS `AuthorizationDecision` types and `intentHash`/`authorizeWithPolicy`/`verifyDecision(ForAction)`/`signWithDecision` wrappers.
- **signet-python**: `intent_hash`, `authorize_with_policy`, `verify_decision(_for_action)`, `sign_with_decision`; `AuthorityMismatchError`, `DecisionInvalidError`.
