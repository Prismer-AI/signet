...
| Hash format | `sha256:<hex>` (always prefixed) | Algorithm-agile: prefix identifies the hash algorithm. All hash fields (`params_hash`, `record_hash`, `prev_hash`) use this format. Verifiers must match the prefix exactly, not just the hex digest. |

### Hash Format Specification

All hash values in Signet use the prefixed format `<algorithm>:<hex_digest>`:

```
sha256:e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855
```

- Algorithm prefix is always lowercase (`sha256`, not `SHA256`)
- Hex digest is always lowercase
- No spaces or padding
- The prefix is part of the canonical representation and MUST be included when
  computing compound hashes (e.g., `record_hash = sha256(JCS({"prev_hash": "sha256:...", ...}))`)

This matters for cross-language verification: a verifier that strips the prefix
before hashing will produce a different result than one that includes it.

### Cross-Language Test Vectors

Implementations MUST pass these edge cases to ensure JCS compatibility:

1. **Float normalization:** `{"value": 1.0}` must canonicalize identically to
   `{"value":1}` (JCS removes trailing zeros)
2. **Non-ASCII:** `{"name": "日本語"}` must produce identical bytes across
   Rust, Python, and JavaScript
3. **Negative zero:** `{"value": -0}` must canonicalize to `{"value":0}`
   (JCS normalizes negative zero)
4. **Key ordering:** `{"z": 1, "a": 2}` must canonicalize to `{"a":2,"z":1}`
