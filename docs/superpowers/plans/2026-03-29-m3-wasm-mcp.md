...
**Goal:** Build @signet/core (TS wrapper for WASM) and @signet/mcp (SigningTransport middleware) npm packages, plus an end-to-end example.

**Architecture:** @signet/core wraps WASM functions with TypeScript types. @signet/mcp implements MCP Transport interface as a decorator that auto-signs tool calls. npm workspaces for monorepo linking.
...
### Task 2: Root package.json + @signet/core package
...
git commit -m "feat(ts): create @signet/core TypeScript wrapper for WASM"
```

---

### Task 3: @signet/core tests
...
git commit -m "test(ts): add 6 unit tests for @signet/core"
```

---

### Task 4: @signet/mcp package + SigningTransport
...
git commit -m "feat(mcp): create @signet/mcp with SigningTransport middleware"
```

---

### Task 5: @signet/mcp tests
...
import { describe, it } from 'node:test';
import assert from 'node:assert';
import { generateKeypair, verify, type SignetReceipt } from '@signet/core';
...
|---|-----------|-------------|
| 1 | @signet/core exports typed functions | Task 3: 6 tests |
| 2 | @signet/mcp exports SigningTransport | Task 5: 5 tests |
